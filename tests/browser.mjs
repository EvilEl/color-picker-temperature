...
try {
  ...
  const initial = await evaluate(`(async () => {
    ...
    other.create('#other', options); other.destroyed();
    ...
    window.fixture = { ColorTemperature, BuildCanvas, Controllers, Component, check, settle, host, picker, canvas, radio, colorAt, options, baselineStyles };
  ...
  await evaluate(`(async () => { const f = fixture; await f.settle(); f.check(f.picker.getColor() === f.colorAt(f.canvas.width - 1), 'Outside drag must select last pixel'); f.check(parseFloat(f.radio.style.left) > 90, 'Marker must reach right edge'); })()`);
